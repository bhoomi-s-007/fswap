...
async def refill(update, context):
    ...
    try:
        ...
        await update.message.reply_text("Added {} credits to user {}.".format(amount, user_id))
...
async def faceswap_image(update, context):
    user_id = update.effective_user.id
...
async def faceswap_video(update, context):
    user_id = update.effective_user.id
